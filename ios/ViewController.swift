...
var mlModel = try! yolo11s(configuration: .init()).model

class ViewController: UIViewController {
  func setupView() {
    videoPreview.frame = view.bounds
    view.addSubview(videoPreview)
    View0.frame = view.bounds
    view.addSubview(View0)
    let centerX = view.bounds.width/2
    labelName.frame = CGRect(x: 0, y: 60, width: view.bounds.width, height: 100)
    view.addSubview(labelName)
    labelName.textAlignment = .center
    labelName.font = .systemFont(ofSize: 24, weight: .medium)
    labelFPS.frame = CGRect(x: 0, y: 160, width: view.bounds.width, height: 100)
